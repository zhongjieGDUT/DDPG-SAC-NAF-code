...
class DDPG(object):
    def __init__(self, gamma, tau, hidden_size, num_inputs, action_space):
        ...
        self.actor = Actor(hidden_size, self.num_inputs, self.action_space).to(device=self.device)
        self.actor_target = Actor(hidden_size, self.num_inputs, self.action_space).to(device=self.device)
        self.actor_perturbed = Actor(hidden_size, self.num_inputs, self.action_space).to(device=self.device)
        self.actor_optim = Adam(self.actor.parameters(), lr=1e-4)

        self.critic = Critic(hidden_size, self.num_inputs, self.action_space).to(device=self.device)
        self.critic_target = Critic(hidden_size, self.num_inputs, self.action_space).to(device=self.device)
    ...
    def select_action(self, state, action_noise=None, param_noise=None):
        ...
        return mu.detach().cpu()   #  .clamp(-1, 1)
