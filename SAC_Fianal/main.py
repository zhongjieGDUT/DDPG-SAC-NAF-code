...
parser.add_argument('--env-name', default="Pendulum-v0",
                    help='Mujoco Gym environment (default: MountainCarContinuous-v0)')
...
if not load:
    for i_episode in itertools.count(1):
        ...
        state = env.reset()
        ounoise.scale = (noise_scale - final_noise_scale) * max(0, 50 - i_episode) / 50 + final_noise_scale
        ounoise.reset()
        while not done:
    ...
    plt.figure(1)
    plt.plot(ep_r_hist)
    plt.savefig('rewards_Pendulum-v0_GaussianV2.0.png')
    agent.save_model(args.env_name, "GaussianV2.0")
else:
    agent.load_model("./models/sac_actor_LunarLanderContinuous-v2_GaussianV2.0", "./models/sac_critic_LunarLanderContinuous-v2_GaussianV2.0")
    for i in range(6):
        state = env.reset()
        t = 0
        ep_reward = 0
